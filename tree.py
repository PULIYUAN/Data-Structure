...
class Tree(object):
    ...
    def delete(self, item):
        # 先获取待删除节点item的父节点（以下简称item）。
        # 如果父节点不为空，判断item的左右子树是否存在：
        #     当左子树为空时，进一步判断item是父节点的左孩子，还是右孩子；
        #         如果是左孩子，将父节点的左指针指向item的右子树，反之将父节点的右指针指向item的右子树。
        #     当右子树为空时，进一步判断item是父节点的左孩子，还是右孩子；
        #         如果是左孩子，将父节点的左指针指向item的左子树，反之将父节点的右指针指向item的左子树。
        #     当左右子树均不为空时，寻找右子树中的最左叶子节点x，将x替代要删除的节点。
        # 删除成功，返回True。
        # 删除失败, 返回False。
        # 如果根为空，就什么也不做
        if self.root is None:
...
if __name__ == '__main__':
    ...
    for i in range(1,11):
        tree1.add(i)
        node = Node(i)
    print("inorder:",tree1.inorder(tree1.root))
    print("postorder:", tree1.postorder(tree1.root))
    print("preorder:", tree1.preorder(tree1.root))